...
`wopr mcp` exposes the 22 core API functions as MCP tools over **stdio** (JSON-RPC 2.0). It is intended for **local, trusted coding agents** — there is no authentication and no remote transport (HTTP/SSE is a follow-up). The agent launches `wopr mcp` as a child process and talks to it over stdin/stdout.
...
wopr mcp --list-tools   # → 22 tool names + descriptions
```

When the agent starts, it will call `tools/list` and receive all 22 tools
...
- **No namespacing.** All 22 tools are flat with `snake_case` names.
