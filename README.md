...
- **Drive wopr from inside pi.** The `wopr-for-pi` extension at `extensions/wopr-for-pi/` registers the same 23 tools as first-class pi-native tools and ships a skill.md that teaches pi *when* and *how* to use them. Install with `pi extensions install ./extensions/wopr-for-pi`.
